...
public record BasicResponse(String response, String error, String detail) {
  ...
  public static ResponseEntity<BasicResponse> to(BusinessException exception) {
    return ResponseEntity.status(exception.getHttpStatus()).body(BasicResponse.of(exception));
  }
}
