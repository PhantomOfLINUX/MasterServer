...
public class PlayerProfileController {
  ...
  public ResponseEntity<PlayerProfile> getMyProfile(@AuthenticationPrincipal Player player) {
    ...
    return ResponseEntity.status(HttpStatus.OK).body(playerProfile);
  }
  ...
  @Operation(
      ...
      description = "자신의 문제풀이 상태를 조회할 수 있다. 자신이란 현재 소유중인 엑세스 토큰에 기반한다.\n\n"
          + "*NOT_COMPLETED*는 허용하지 않는다." + " *uri 정보는 현재 제공되지 않는다.*")
  public ResponseEntity<PlayerStageProgressResponse> getStagesByStatusForPlayer(
      @AuthenticationPrincipal Player player, @RequestParam CompletedStatus status) {
  ...
  @Operation(summary = "자신의 문제풀이 heatmap 정보 조회", description = "자신의 문제풀이 내역에 대한 heatmap 정보를 조회한다")
  public ResponseEntity<List<HeatMapDataPoint>> getHeatMapPointsForPlayer(
      @AuthenticationPrincipal Player player) {
    List<HeatMapDataPoint> response =
        playerProfileService.getHeatMapDataPointsByModifiedDate(player);

    return ResponseEntity.status(HttpStatus.OK).body(response);
  }

  @Operation(summary = "관리자 권한 확인", description = "해당 유저가 관리자 권한을 가지고 있는지 검증합니다.")
  @LogMonitoring
  @GetMapping("me/admin")
