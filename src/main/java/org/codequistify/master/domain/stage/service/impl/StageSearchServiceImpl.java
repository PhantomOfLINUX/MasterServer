...
public class StageSearchServiceImpl implements StageSearchService {
  ...
  public Stage getStageByCode(StageCode stageCode) {
    ...
    return stageRepository.findByStageImage(stageImage).orElseThrow(() -> {
  ...
  public QuestionResponse findQuestion(Long stageId, Integer questionIndex) {
    Question question = questionRepository
        .findByStageIdAndIndex(stageId, questionIndex)
        .orElseThrow(() -> {
          LOGGER.info(
              "[findQuestion] {}, id: {}, index: {}",
  ...
  public StageResponse getStageByChoCho(String query) {
    ...
    return stageConverter.convert(stageRepository.findAll().stream()
        .parallel()
        .filter(stage -> {
  ...
  public StageResponse getStageBySearchText(String query) {
    return stageConverter.convert(stageRepository.findAll().stream()
        .parallel()
        .filter(stage -> {
          if (stage.getTitle().toLowerCase().contains(query.toLowerCase())) {
            return true;
          }
          if (stage.getDescription().toLowerCase().contains(query.toLowerCase())) {
            return true;
          }
          if (stage.getStageImage().name().toLowerCase().contains(query.toLowerCase())) {
            return true;
          }
          return false;
        })
  ...
  public PlayerStageProgressResponse getCompletedStagesByPlayerId(PlayerId playerId) {
    return new PlayerStageProgressResponse(
        completedStageRepository.findCompletedStagesByPlayerId(playerId.value()));
  }
  ...
  public PlayerStageProgressResponse getInProgressStagesByPlayerId(PlayerId playerId) {
    return new PlayerStageProgressResponse(
        completedStageRepository.findInProgressStagesByPlayerId(playerId.value()));
  }
  ...
  public StagePageResponse findStagesByCriteria(SearchCriteria searchCriteria, Player player) {
    PageRequest pageRequest =
        PageRequest.of(searchCriteria.getPage_index() - 1, searchCriteria.getPage_size());
    ...
    this.addWhereConditionForCompletedStatus(
        whereClause, searchCriteria, qCompletedStage, player.getId());

    // 임시 스테이지는 제외
    whereClause.and(qStage.approved.eq(true));

    // 쿼리 결과 조회
    List<StageResponse> results =
        fetchStageResponses(whereClause, qStage, qCompletedStage, pageRequest, player.getId());
  ...
  private long fetchTotalCount(
      BooleanBuilder whereClause, QStage qStage, QCompletedStage qCompletedStage, Long playerId) {
    return queryFactory
        .from(qStage)
        .leftJoin(qStage.completedStages, qCompletedStage)
        .on(qCompletedStage.player.id.eq(playerId))
  ...
  private void addWhereConditionForStageGroupTypes(
      BooleanBuilder whereClause, SearchCriteria searchCriteria, QStage qStage) {
    if (searchCriteria.getStageGroupTypes() == null
        || searchCriteria.getStageGroupTypes().isEmpty()) {
      return;
    }
    whereClause.and(qStage.stageGroup.in(searchCriteria.getStageGroupTypes()));
  }

  private void addWhereConditionForDifficultLevels(
      BooleanBuilder whereClause, SearchCriteria searchCriteria, QStage qStage) {
    if (searchCriteria.getDifficultyLevels() == null
        || searchCriteria.getDifficultyLevels().isEmpty()) {
      return;
    }
  ...
  private void addWhereConditionForSearchText(
    ...
    BooleanExpression descriptionContains =
        qStage.description.contains(searchCriteria.getSearchText());

    BooleanExpression titleContains = qStage.title.contains(searchCriteria.getSearchText());

    BooleanExpression stageImageContains = Expressions.stringTemplate(
            "cast({0} as string)", qStage.stageImage)
        .contains(searchCriteria.getSearchText());

    whereClause.and(descriptionContains.or(titleContains).or(stageImageContains));
  }

  private void addWhereConditionForCompletedStatus(
      BooleanBuilder whereClause,
    ...
    if (searchCriteria.getCompleted() != null) {
      if (searchCriteria.getCompleted() == CompletedStatus.NOT_COMPLETED) {
        ...
        whereClause.and(qCompletedStage
            .status
            .eq(searchCriteria.getCompleted())
