...
public class StageConverter {
  ...
  public Stage convert(StageRegistryRequest request) {
    return Stage.builder()
        ...
        .questions(IntStream.range(0, request.questions().size())
            .mapToObj(index -> {
              Question question = questionConverter.convert(request.questions().get(index));
  ...
  public static List<StageResponse> convert(List<Stage> stages) {
    return stages.stream().map(StageConverter::convert).collect(Collectors.toList());
  }
