...
public class StageManagementController {
  ...
  // 문항 채점 요청
  @Operation(summary = "문항 채점 요청", description = """
  ...
  public ResponseEntity<GradingResponse> submitAnswerForGrading(
      @AuthenticationPrincipal Player player, @Valid @RequestBody GradingRequest request) {
  ...
  // 다음 문항 구성
  @Operation(summary = "다음 문제 환경 구성", description = "다음 문제를 풀기 위해 필요한 환경을 구성합니다.")
  @LogMonitoring
  @PostMapping("question/compose")
  public ResponseEntity<BasicResponse> compose(
      @AuthenticationPrincipal Player player, @Valid @RequestBody GradingRequest request) {
    validatePlayer(player, request);
    SuccessResponse successResponse =
        stageManagementService.composeVirtualWorkspace(player, request);
  ...
  // 문제 풀이 완료 요청
  @Operation(summary = "스테이지 풀이 완료 기록", description = "`stage_id` 에 대한 stage 완료를 기록합니다.")
  @PostMapping("stages/{stage_id}/complete")
  @LogMonitoring
  public ResponseEntity<StageCompletionResponse> completeStage(
