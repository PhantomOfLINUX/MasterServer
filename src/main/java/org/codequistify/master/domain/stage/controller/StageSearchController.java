...
public class StageSearchController {
  ...
  // 스테이지 목록 조회
  @Operation(summary = "스테이지 목록 조회", description = """
  ...
  public ResponseEntity<StageResponse> getStageInfoById(
  ...
  @Operation(summary = "문항 조회", description = "문항을 조회합니다. 'stage 정보'와 '몇 번째 문항' 인지에 대한 정보로 조회합니다.")
  // 스테이지 문항 조회 -> 정답은 클라이언트에게 제공 안 됨, 옵션들은 전부 제공되어야 함
  @LogMonitoring
  @GetMapping("stages/{stage_id}/questions/{question_index}")
  public ResponseEntity<QuestionResponse> getQuestionByStage(
  ...
  public ResponseEntity<HangulExtractor.ChoCho> HangulQuery(@RequestParam String src) {
    ...
    HangulExtractor.ChoCho response =
        new HangulExtractor.ChoCho(hangulExtractor.extractChoseongs(src));
