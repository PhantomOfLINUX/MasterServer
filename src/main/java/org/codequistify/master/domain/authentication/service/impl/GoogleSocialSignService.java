...
public class GoogleSocialSignService implements SocialSignService {
  ...
  public String getSocialLogInURL() {
    return "https://accounts.google.com/o/oauth2/auth?" + "client_id="
        + oAuthKey.getGOOGLE_CLIENT_ID() + "&redirect_uri="
        + oAuthKey.getGOOGLE_REDIRECT_URI() + "&response_type=code"
  ...
  private OAuthToken getOAuthToken(String code) {
    ...
    try {
      OAuthToken response =
          restTemplate.postForObject(oAuthKey.getGOOGLE_TOKEN_URI(), entity, OAuthToken.class);
  ...
  private OAuthResource getUserResource(String accessToken) {
    ...
    try {
      return restTemplate
          .exchange(oAuthKey.getGOOGLE_RESOURCE_URI(), HttpMethod.GET, entity, OAuthResource.class)
          .getBody();
