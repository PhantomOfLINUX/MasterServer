...
public class KakaoSocialSignService implements SocialSignService {
  ...
  public String getSocialLogInURL() {
    return "https://kauth.kakao.com/oauth/authorize?response_type=code" + "&client_id="
        + oAuthKey.getKAKAO_CLIENT_ID() + "&redirect_uri="
        + oAuthKey.getKAKAO_REDIRECT_URI();
  }
  ...
  public PlayerProfile socialLogIn(OAuthData oAuthData) {
    ...
    Player player =
        playerDetailsService.findOnePlayerByEmail(oAuthData.resource().email());
  ...
  private OAuthToken getOAuthToken(String code) {
    ...
    try {
      OAuthToken response =
          restTemplate.postForObject(oAuthKey.getKAKAO_TOKEN_URI(), entity, OAuthToken.class);
      LOGGER.info("[getOAuthToken] token 정보{}", response);
      return response;
    } catch (RestClientException exception) {
      LOGGER.info("[getOAuthToken] 토큰 요청 실패");
      throw new BusinessException(
          ErrorCode.OAUTH_COMMUNICATION_FAILURE, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  private OAuthResource getUserResource(String accessToken) {
    ...
    try {
      Map<String, String> map = restTemplate
          .exchange(oAuthKey.getKAKAO_RESOURCE_URI(), HttpMethod.GET, entity, ResourceOfKakao.class)
          .getBody()
          .properties();
      OAuthResource response =
          new OAuthResource(map.get("id"), map.get("email"), map.get("nickname"));
