...
public class AuthenticationController {
  ...
  public ResponseEntity<LoginResponse> socialSignInGoogle(
      @RequestBody SocialLogInRequest request, HttpServletResponse response) {
  ...
  public ResponseEntity<LoginResponse> socialLogInKakao(
      @RequestBody SocialLogInRequest request, HttpServletResponse response) {
  ...
  public ResponseEntity<LoginResponse> socialLogInNaver(
      @RequestBody SocialLogInRequest request, HttpServletResponse response) {
  ...
  public ResponseEntity<LoginResponse> socialLogInGithub(
      @RequestBody SocialLogInRequest request, HttpServletResponse response) {
  ...
  @Operation(summary = "POL 자체 회원가입", description = """
  ...
  public ResponseEntity<LoginResponse> signUpPOL(
      @Valid @RequestBody SignUpRequest request, HttpServletResponse response) {
    // 이메일 인증 정보를 확인
    EmailVerification emailVerification = emailVerificationService.verifyEmailAndRetrieve(
        request.email(), EmailVerificationType.REGISTRATION);
  ...
  @Operation(summary = "POL 자체 로그인", description = "자체 로그인기능이다. name, password를 필수로 입력받는다.")
  @LogMonitoring
  @PostMapping("auth/login")
  public ResponseEntity<LoginResponse> logInPOL(
  ...
  public ResponseEntity<TokenResponse> regenerateAccessToken(
  ...
  public ResponseEntity<BasicResponse> LogOut(
    ...
    return ResponseEntity.status(HttpStatus.OK).body(BasicResponse.of("SUCCESS"));
  }
  ...
  @Operation(summary = "인증메일 발송 요청", description = """
  ...
  @LogMonitoring
  public ResponseEntity<BasicResponse> sendAuthMail(
    ...
    emailVerificationService.sendVerifyMail(
        request.email(), request.type()); // 비동기 요청 : 메일 전송에 시간이 너무 오래걸림
  ...
  @Operation(
      ...
      description =
          "회원가입 인증코드를 확인한다. 사용자의 email과 입력 코드를 path로 받는다. 결과는 response 필드에 \"true\", \"false\"로 반환한다.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "인증 결과",
  ...
  public ResponseEntity<BasicResponse> verifyCode(
    ...
    return ResponseEntity.status(HttpStatus.OK).body(BasicResponse.of(bool));
  }
  ...
  private TokenResponse generateTokens(PlayerProfile playerProfile) {
    ...
    authenticationService.updateRefreshToken(
        playerProfile.uid(), refreshToken); // refresh token db에 저장
