...
public class GithubSocialSignService implements SocialSignService {
  ...
  public String getSocialLogInURL() {
    return "https://github.com/login/oauth/authorize?" + "client_id="
        + oAuthKey.getGITHUB_CLIENT_ID() + "&redirect_uri="
  ...
  private OAuthToken getOAuthToken(String code) {
    ...
    try {
      OAuthToken response =
          restTemplate.postForObject(oAuthKey.getGITHUB_TOKEN_URI(), entity, OAuthToken.class);
  ...
  private OAuthResource getUserResource(String accessToken) {
    ...
    try {
      Map<String, String> map = restTemplate
          .exchange(oAuthKey.getGITHUB_RESOURCE_URI(), HttpMethod.GET, entity, HashMap.class)
          .getBody();
      OAuthResource response =
          new OAuthResource(String.valueOf(map.get("id")), map.get("login"), map.get("name"));
