...
public class NaverSocialSignService implements SocialSignService {
  ...
  public String getSocialLogInURL() {
    return "https://nid.naver.com/oauth2.0/authorize?" + "client_id="
        + oAuthKey.getNAVER_CLIENT_ID() + "&redirect_uri="
  ...
  private OAuthToken getOAuthToken(String code) {
    ...
    try {
      OAuthToken response =
          restTemplate.postForObject(oAuthKey.getNAVER_TOKEN_URI(), entity, OAuthToken.class);
  ...
  private OAuthResource getUserResource(String accessToken) {
    ...
    try {
      Map<String, String> map = restTemplate
          .exchange(oAuthKey.getNAVER_RESOURCE_URI(), HttpMethod.GET, entity, ResourceOfNaver.class)
          .getBody()
          .response();
