...
public class VirtualWorkspaceController {
  ...
  @Operation(summary = "가상 작업공간 (VirtualWorkspace) 생성요청", description = """
  ...
  @LogMonitoring
  @PostMapping("/stages")
  public ResponseEntity<VirtualWorkspaceConnectionResponse> applyVirtualWorkspace(
    ...
    if (lock.tryLock()) {
      try {
        VirtualWorkspaceConnectionResponse response =
            virtualWorkspaceService.recreate(request.stageCode(), player);
  ...
  @Operation(summary = "가상 작업공간 (VirtualWorkspace) 조회", description = """
                    :stageCode에 대한 VirtualWorkspace 상태 정보를 조회한다.
                    """)
  @GetMapping("/stages/{stageCode}")
  @LogMonitoring
  public ResponseEntity<VirtualWorkspaceSummaryResponse> getVirtualWorkspace(
      @AuthenticationPrincipal Player player, @NotNull @PathVariable String stageCode) {
    VirtualWorkspaceSummaryResponse response =
        virtualWorkspaceService.getSummary(stageCode, player);

    return ResponseEntity.status(HttpStatus.OK).body(response);
  }

  @Operation(summary = "가상 작업공간 (VirtualWorkspace) 상태 조회", description = """
                    :stageCode에 대한 VirtualWorkspace 상태를 확인한다.
                    """)
  @GetMapping("/stages/{stageCode}/status")
  public ResponseEntity<VirtualWorkspaceStatusResponse> getVirtualWorkspaceStatus(
      @AuthenticationPrincipal Player player, @NotNull @PathVariable String stageCode) {
  ...
  @Operation(summary = "가상 작업공간 (VirtualWorkspace) 연결 정보 조회", description = """
                    :stageCode에 대한 VirtualWorkspace 연결 정보를 조회한다.
                    """)
  @GetMapping("/stages/{stageCode}/connection")
  public ResponseEntity<VirtualWorkspaceConnectionResponse> getVirtualWorkspaceConnection(
      @AuthenticationPrincipal Player player, @NotNull @PathVariable String stageCode) {
    VirtualWorkspaceConnectionResponse response =
        virtualWorkspaceService.getConnection(stageCode, player);
