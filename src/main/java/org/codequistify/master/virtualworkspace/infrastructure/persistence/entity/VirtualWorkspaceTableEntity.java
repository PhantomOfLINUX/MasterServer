...
public class VirtualWorkspaceTableEntity extends BaseTimeEntity {
  ...
  public static VirtualWorkspaceTableEntity fromDomain(VirtualWorkspace workspace) {
    ...
    return VirtualWorkspaceTableEntity.builder()
        ...
        .k8sNamespace(
            internalRoute.map(VirtualWorkspaceInternalRoute::namespace).orElse(null))
        .serviceName(
            internalRoute.map(VirtualWorkspaceInternalRoute::serviceName).orElse(null))
        .servicePort(
            internalRoute.map(VirtualWorkspaceInternalRoute::servicePort).orElse(null))
        .image(spec.image())
        .port(spec.port())
        .cpu(spec.cpu())
        .memory(spec.memory())
  ...
  public VirtualWorkspace toDomain() {
    VirtualWorkspaceId id = VirtualWorkspaceId.of(PlayerId.of(playerId), StageCode.from(stageCode));
    ...
    StageSpecSnapshot spec =
        StageSpecSnapshot.of(StageCode.from(stageCode), image, port, cpu, memory, readinessPath);

    VirtualWorkspacePublicEndpoint endpoint =
        VirtualWorkspacePublicEndpoint.of(publicIdVo, baseHost);
    ...
    WorkspaceAccessPolicy accessPolicy =
        new WorkspaceAccessPolicy(SubjectId.from(ownerSubjectId), accessMode);
